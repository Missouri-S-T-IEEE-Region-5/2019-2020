...
for i=1:length
    if i==1 % Initial value is midpoint 1 if bot resets to nearest 90degs before scan.
...
% Figure out how to fill in missing midpoints and corners because of the bot
...
for i=1:lengthsd % Go through stored points based on what section it is in
    if senddata(2,7)>=senddata(2,i) && senddata(2,3)<=senddata(2,i) % Evaluate section 1
        if i==3 && not(senddata(1,i)==0) && not(senddata(1,i)==0) % Convert midpoint 1 if not missing
   ...
   if senddata(2,5)>=senddata(2,i) && senddata(2,8)<=senddata(2,i) % Evaluate section 4
        ...
        if not(i==5) && not(i==8) && not(senddata(1,i)==0) % If bot wall present convert it if not missing
           xyresult(1,i)=senddata(1,5)-(senddata(1,i)*sind(180-senddata(2,i)));
           xyresult(2,i)=senddata(1,6)+(senddata(1,i)*cosd(180-senddata(2,i)));
        end
   end
...
%   end
    ...
    if 360>=senddata(2,i) && senddata(2,10)<=senddata(2,i) % Evaluate section 8
        ...
        if not(i==10) && not(senddata(1,i)==0) % If bot wall present convert it if not missing
           xyresult(1,i)=senddata(1,6)-(senddata(1,i)*sind(360-senddata(2,i)));
           xyresult(2,i)=senddata(1,5)+(senddata(1,i)*cosd(360-senddata(2,i)));
        end
   end
end
